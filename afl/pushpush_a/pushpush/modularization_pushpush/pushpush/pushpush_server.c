...
//disconnect: check if any user is disconnect, handel clnt_cnt
void disconnect(int sock)
{
	fprintf(stderr,"This is disconntected\n");
	pthread_mutex_lock(&mutx);
	for (int i = 0; i < clnt_cnt; i++)   // remove disconnect client
	{
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2) {
		printf("Usage : %s <port> <mapdatapath>\n", argv[0]);
		exit(1);
	}

	if(loadJson(argv[2])) //parsing error return 1.
		exit(1);
